...
//         BloodPressureCriticalRule rule = new BloodPressureCriticalRule();
...
//         );
//         assertFalse(rule.isExceeded(normal), "Normal blood pressure should not trigger an alert.");

//         // Critical High Systolic (> 180)
//         assertTrue(rule.isExceeded(Collections.singletonList(
...
//         );
//         assertTrue(rule.isExceeded(increasing), "3 consecutive increasing pressure values should trigger alert.");
...
//         );
//         assertTrue(rule.isExceeded(decreasing), "3 consecutive decreasing diastolic pressure values should trigger alert.");
...
//         );
//         assertFalse(rule.isExceeded(stable), "Stable blood pressure should not trigger alert.");
//     }

//     // blooad saturation rule tests
//     @Test
//     void testBloodSaturationRule() {
//         BloodSaturationRule rule = new BloodSaturationRule();
//         long t = System.currentTimeMillis();
...
//         );
//         assertTrue(rule.isExceeded(lowSat), "Saturation < 92% should trigger alert.");
//         assertEquals("Low Blood Saturation Alert", rule.getConditionName());
...
//         );
//         assertTrue(rule.isExceeded(dropSat), "Saturation drop of 5% should trigger alert.");
//         assertEquals("Rapid Saturation Drop Alert", rule.getConditionName());
...
//         );
//         assertFalse(rule.isExceeded(normalSat), "Normal saturation should not trigger alert.");
//     }

//     // ECG anomaly tests
//     @Test
//     void testECGAbnormalRule() {
//         ECGAbnormalRule rule = new ECGAbnormalRule();
//         long t = System.currentTimeMillis();
...
//         );
//         assertFalse(rule.isExceeded(normalEcg), "Stable ECG should not trigger alert.");
...
//         );
//         assertTrue(rule.isExceeded(abnormalEcg), "Peak exceeding 20% of average should trigger alert.");
//     }
...
//                 new PatientRecord(1, 91, "Saturation", t)
//         );
//         assertTrue(rule.isExceeded(triggered), "Concurrent low systolic and saturation should trigger alert.");
...
//         );
//         assertFalse(rule.isExceeded(partial), "Only one condition met should NOT trigger alert.");
//     }

//     // manual trigger rule tests
//     @Test
//     void testManualTriggerRule() {
//         ManualTriggerRule rule = new ManualTriggerRule();
//         long t = System.currentTimeMillis();
...
//         assertFalse(rule.isExceeded(Collections.singletonList(
